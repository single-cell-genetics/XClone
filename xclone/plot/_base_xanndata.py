...
def multi_lut_set(Xdata, cell_anno_key, clusters_display_name, sns_palette_lst):
    ...
    for anno_key, anno_display_name, sns_palette in zip(cell_anno_key,clusters_display_name, sns_palette_lst):
        ...
        # Reset index if it's a MultiIndex to avoid pandas compatibility issues with isna
        if isinstance(_clusters.index, pd.MultiIndex):
            _clusters = _clusters.reset_index(drop=True)
        # Convert categorical to string to avoid MultiIndex category issues
        if pd.api.types.is_categorical_dtype(_clusters):
            _clusters = _clusters.astype(str)
